...
class BatchingAlarmStore:
    ...
    def __init__(self,TIME_OVERLAP_PRIORITY,HARDWARE_SET_PRIORITY):
        self.TIME_OVERLAP_PRIORITY = TIME_OVERLAP_PRIORITY
        self.HARDWARE_SET_PRIORITY = HARDWARE_SET_PRIORITY
        self.mAlarmBatches = []
        self.mNum = 0
    ...
    def insertAndBatchAlarm(self, alarm):
        whichBatch = -1 if (alarm.flags & FLAG_STANDALONE != 0) \
            else self.attemptCoalesce(alarm.getWhenElapsed(),alarm.getMaxWhenElapsed(),alarm.uid)
        if whichBatch < 0:
